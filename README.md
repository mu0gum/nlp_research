...
## 5. 선물 추천
 너무 오랜만에 글을 다시 쓰는 것 같습니다. 챗봇을 만들면고 자연어 처리를 공부하면서 기초가 없으면 안되겠다는 생각이 들었습니다. 지금은 확률과 통계 공부도 시작하고 해서 글을 천천히 쓸 시간이 없었습니다ㅜㅜ 오늘은 제가 생각한 Word2Vec을 활용한 선물 추천 알고리즘을 다른 분께 설명하고 피드백을 받아 볼 기회가 있었습니다. 피드백을 받고 보니 피드백 받은 내용을 기록해두고 나중에 적용해야겠다는 생각을 했습니다. 그래서 이렇게 다시 글을 쓰게 되었네요! 그럼 시작하겠습니다. 일단 제가 생각한 선물 추천 알고리즘의 아이디어는 이렇습니다.

```
1. 인스타그램에서 "선물" 로 게시글 검색
2. 선물과 관련된 게시글 크롤링
3. 크롤링 한 데이터를 형태소 분석하고 불필요한 데이터를 제거 or 명사만 추출
4. 정리된 데이터를 Word2Vec으로 임베딩
5. Word2Vec 으로 학습된 모델에 코사인 유사도가 가장 가까운 단어들을 검색
 => model.most_similar(positive=['엄마', '생신', '선물'])
6. 나온 결과에서 선물 데이터만 추출
```
 
 아래는 피드백을 주신 부분입니다.

```
1. 왜 인스타그램에서 데이터를 가지고 왔는지? 구글에서 검색해서 가져 오는건 어떤지
2. Word2Vec으로 학습시킨 데이터의 양은 얼마인지? ( 2만건의 데이터는 너무 적다. 최소 100 배는 되어야 함 )
```

 오늘 피드백을 받으면서 좀 느꼈던 점은 하나의 문제에 대해 조금 깊게 고민해보지 못한게 아닌가라는 생각을 했습니다. 너무 이것저것 할 게 많아서 지금은 막 벌려놓은 상황인데, 이제는 벌려 놓은 것들을 하나씩 정리하면서 그 깊이에 대해서도 함께 고민해 보겠습니다. 일단은 피드백 받기 전의 내용을 바탕으로 결과를 보여드리고, 피드백 받은 내용과 더불어 좀 더 깊이 생각해서 문제에 대해 접근도 해보겠습니다.
