...
### 중간 점검
 - Naver AI Hackathon 2019 서류에서 탈락했습니다. 처음에는 그냥 한 번 해보자는 가벼운 마음으로 했는데, 점점 공부하면서 혼자 기간 내에 할 수 있을까? 괜히 100팀 안에 들면 정말 필요한 사람에게 기회가 안 가게 되는게 아닌가라는 걱정을 했는데 너무 앞서 나갔네요..ㅎㅎㅎ해커톤 자체도 참여해보고 싶었고, AI 관련 해커톤이라 더더욱 참여하고 싶었지만 아직 역량이 부족하다는 점을 다시 한 번 확인했습니다.
 - 이번에 준비하면서 개인적인 목표인 논문을 코드로 구현해보는 것은 해커톤과는 별개로 계속 진행할 생각입니다. 다만 지금은 우선순위로 해야할 것이 있어서 그 부분을 마무리하고 다시 시작할 생각입니다. 또 한 번 벽을 느끼고 조금은 우울한 기분도 들었지만 역시 재미있고 다시 도전해보고 싶은 생각이 들었습니다. 
 - 그리고 음성인식을 하는 회사 중에서 일상대화 외에도 특수한 상황에서의 대화를 따로 음성인식하는 부분이 있었습니다. 음성인식에 대해 전혀 몰랐을 때는 '그냥 음성인식만 잘 하면 되는데 왜 상황별로을 구분지었을까?'라는 궁금증이 있었는데, 지금 생각해보면 아마 언어모델 부분과 연관되어 있어서 그렇지 않을까 생각합니다. 아무래도 특정 상황에서 더 많이 사용되는 단어가 있을 때 그 단어에 대한 확률값을 더 크게 가져가면 좀 더 정확한 인식이 되기 때문에 아닌가 추측해보았습니다. 공부를 하면서 이렇게 알게된 내용을 바탕으로 무언가를 추측해보고 확인하고(확인은 하지 못했지만..ㅠㅠ) 하는 행동들이 정말 재미있는 것 같습니다.ㅎㅎ얼른 Word2Vec을 활용한 추천 모델을 개발 및 검토 해보고 다시 음성인식 모델을 개발해보도록 하겠습니다~!
